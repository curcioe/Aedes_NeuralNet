...
absence_ratios = [0.1, 0.2, 0.3, 0.4]
kfold_patience = 10
final_model_patience = 100
batch_size = 32
learning_rate = 0.001
k_folds = 5
sequence_length = 30  # Days range from -365 to -1

# File paths and folders
data_file = 'C:/Users/Owner/Desktop/Gift4Evan/Gift4Evan/datasets/aedes_aegypti_dataset.csv'
...
start_date_str = "2020-12-01"  # Replace with your desired start date
...
# Identify the columns that represent the weather variables
weather_columns = data.columns[4:]  # The first 4 columns are non-weather columns
weather_var_types = sorted(set([col.split('.')[1] for col in weather_columns]))  # e.g., ['mean_2m_air_temperature', 'range_2m_air_temperature', ...]

# Initialize scalers for each weather variable type
scalers = {var: StandardScaler() for var in weather_var_types}

# Batch standardize the weather variables
...
def prepare_ffnn_data(df, weather_columns, sequence_length):
    X = []
    y = []
    for i in range(len(df)):
        weather_sequence = df.iloc[i][weather_columns].values.reshape(sequence_length, len(weather_var_types)).astype(np.float32)
        X.append(weather_sequence.flatten())  # Flatten the sequence into a single vector
...
for absence_ratio in absence_ratios:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(remaining_presence)):
        ...
        X_train, y_train = prepare_ffnn_data(train_set, weather_columns, sequence_length)
        X_val, y_val = prepare_ffnn_data(val_set, weather_columns, sequence_length)
...
X_final, y_final = prepare_ffnn_data(final_train_set, weather_columns, sequence_length)
...
X_test, y_test = prepare_ffnn_data(test_set, weather_columns, sequence_length)
...
# Reshape SHAP values for plotting (365 days, 6 weather variables)
shap_values_reshaped = shap_values.reshape(-1, 365, len(weather_var_types))

# Calculate mean absolute SHAP values across all samples
mean_shap = np.abs(shap_values_reshaped).mean(axis=0)

# Create a DataFrame for plotting
weather_var_names = weather_var_types  # Use the weather variable names as the columns
shap_df = pd.DataFrame(mean_shap, columns=weather_var_names, index=range(-365, 0))

# Save SHAP values to a file
shap_values_df = pd.DataFrame(shap_values_reshaped.reshape(-1, 365 * len(weather_var_types)), columns=[f"{var}_{day}" for day in range(-365, 0) for var in weather_var_names])
...
#             scaled_data.append(scalers[var].transform(era5_data[:, [i]]).flatten())
